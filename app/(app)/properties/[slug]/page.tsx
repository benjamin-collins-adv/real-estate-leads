...
export async function generateMetadata({ params }: PropertyPageProps): Promise<Metadata> {
  ...
  const statusText = property.status === "for-sale" ? "For Sale" : "For Rent";
  ...
  const description = property.description || `${property.title} - ${statusText} ${priceText}. ${property.bedrooms} bed, ${property.bathrooms} bath, ${property.sqft.toLocaleString()} sqft. Located in ${property.address}`;

  return {
    title: `${property.title} | ${statusText} ${priceText}`,
    description: description,
    keywords: [
      property.type,
      statusText.toLowerCase(),
      property.address.split(",")[0], // city
      property.address.split(",")[1]?.trim(), // state
      "real estate",
      "property",
      "for sale",
      "for rent",
    ...
    other: {
      ...
      "property:address:street": property.address.split(",")[0] || "",
      "property:address:locality": property.address.split(",")[1]?.trim() || "",
      "property:address:region": property.address.split(",")[2]?.trim() || "",
    },
  };
}
      ...
      <script
        ...
        dangerouslySetInnerHTML={{
          __html: JSON.stringify({
            ...
            "@type": property.status === "for-sale" ? "Product" : "Accommodation",
            name: property.title,
            description: property.description || `${property.title} located in ${property.address}`,
            image: property.image,
            address: {
              "@type": "PostalAddress",
              streetAddress: property.address.split(",")[0] || "",
              addressLocality: property.address.split(",")[1]?.trim() || "",
              addressRegion: property.address.split(",")[2]?.trim() || "",
              postalCode: property.address.split(",")[3]?.trim() || "",
            },
